...
namespace EscolaDeIdioma
{
    internal class DAOCategoria
    {
        ...
        public void Inserir(string nome, int cargaHoraria, double valor)
        {
            try
            {
                dados = $"('','{nome}', '{cargaHoraria}', '{valor}')";
                comando = $"Insert into curso(codigo, nome, cargaHoraria, valor) values{dados}";
